...
class TestIndividual:
    ...
    def test_individual_creation(self):
        ...
        assert (
            individual.embedding is None
        )  # Embedding is None by default until computed
...
class TestPopulation:
    ...
    def test_population_creation(self):
        ...
        population = Population(
            individuals=individuals, target_barycenter=target_barycenter
        )
...
class TestEvolutionaryAlgorithm:
    ...
    def test_evaluate_fitness(self, mock_vecbook_index):
        ...
        assert fitness == 0.85
        mock_vecbook_index.compare_against_barycenter.assert_called_once_with(
            ["test string"]
        )
...
class TestCharGenome:
    """Char-mode genome wiring (the default codec)."""
    ...
    def test_individual_derives_char_genome_from_text(self):
        ...
        ind = Individual(text="abc")
        assert ind.genome == ["a", "b", "c"]

    def test_crossover_and_mutate_preserve_char_text(self):
        ea = self._ea()
        ea.output_length = 12
        child = ea._crossover(
            Individual(text="hello world"), Individual(text="goodbye all")
        )
        assert isinstance(child.text, str) and child.text
        assert child.genome == list(child.text)
        mutated = ea._mutate(Individual(text="hello world"))
        assert mutated.genome == list(mutated.text)
